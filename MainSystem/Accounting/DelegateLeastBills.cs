    ...
    public partial class DelegateLeastBills : Form
    {
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string query = "select distinct concat(customer_bill.Customer_Name,' ',customer_bill.Customer_ID) as 'مهندس/مقاول', concat(customer_bill.Client_Name,' ',customer_bill.Client_ID) as 'العميل' ,Bill_Date as 'تاريخ الفاتورة',customer_bill.Branch_BillNumber as 'رقم الفاتورة',Total_CostAD as 'اجمالي الفاتورة',case when Transition ='ايداع' then Amount end  as 'السداد',case when Transition ='سحب' then Amount  end as 'مرتد السداد'   from customer_bill inner join transitions on customer_bill.Branch_BillNumber=transitions.Bill_Number inner join product_bill on product_bill.CustomerBill_ID=customer_bill.CustomerBill_ID inner join customer_return_bill on transitions.Bill_Number=customer_return_bill.Branch_BillNumber where Paid_Status=2 and transitions.Branch_ID=" + txtBranchID.Text+ " and Delegate_ID="+txtDelegateID.Text+ " and customer_bill.Type_Buy='كاش' and Bill_Date between '" + d+"' and '"+d2+ "' ";
                MySqlDataAdapter da = new MySqlDataAdapter(query, dbconnection);
                DataTable dt = new DataTable();
                da.Fill(dt);
