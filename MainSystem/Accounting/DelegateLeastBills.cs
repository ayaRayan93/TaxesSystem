        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtBranchID.Text != "")// && txtDelegateID.Text != "")
                {
                    ...
                    {
                        query = "select distinct concat(customer_bill.Customer_Name,' ',customer_bill.Customer_ID) as 'مهندس/مقاول', concat(customer_bill.Client_Name,' ',customer_bill.Client_ID) as 'العميل' ,delegate.Delegate_Name as 'المندوب',Bill_Date as 'تاريخ الفاتورة',customer_bill.Branch_BillNumber as 'رقم الفاتورة',Total_CostAD as 'اجمالي الفاتورة',sum(DISTINCT Amount) as 'السداد' ,(Total_CostAD-sum(DISTINCT Amount))as 'الباقي'  from customer_bill inner join transitions on customer_bill.Branch_BillNumber=transitions.Bill_Number inner join product_bill on product_bill.CustomerBill_ID=customer_bill.CustomerBill_ID inner join delegate on delegate.Delegate_ID=product_bill.Delegate_ID  where customer_bill.CustomerBill_ID in(" + str+") and Bill_Date between '" + d + "' and '" + d2 + "' and transitions.Branch_ID="+txtBranchID.Text+ " and Transition='ايداع' group by customer_bill.CustomerBill_ID ";
                    }
                    else
                    {
                        query = "select distinct concat(customer_bill.Customer_Name,' ',customer_bill.Customer_ID) as 'مهندس/مقاول', concat(customer_bill.Client_Name,' ',customer_bill.Client_ID) as 'العميل' ,Bill_Date as 'تاريخ الفاتورة',customer_bill.Branch_BillNumber as 'رقم الفاتورة',Total_CostAD as 'اجمالي الفاتورة',sum(DISTINCT Amount) as 'السداد',(Total_CostAD-sum(DISTINCT Amount))as 'الباقي'   from customer_bill inner join transitions on customer_bill.Branch_BillNumber=transitions.Bill_Number inner join product_bill on product_bill.CustomerBill_ID=customer_bill.CustomerBill_ID inner join delegate on delegate.Delegate_ID=product_bill.Delegate_ID  where customer_bill.CustomerBill_ID in(" + str + ") and product_bill.Delegate_ID=" + txtDelegateID.Text + "  and Bill_Date between '" + d + "' and '" + d2 + "' and transitions.Branch_ID=" + txtBranchID.Text + " and Transition='ايداع' group by customer_bill.CustomerBill_ID";
                    }
                    MySqlDataAdapter da = new MySqlDataAdapter(query, dbconnection);
