...
{
    public partial class Delegate_Movement_Copy3 : Form
    {
        ...
        private void gridView1_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            try
            {
                if (loaded && gridView1.RowCount > 0)
                {
                    ...
                    {
                        bool notbusyflag = false;

                        for (int i = 0; i < gridView1.RowCount; i++)
                        {
                            if (gridView1.GetRowCellValue(i, colStatus).ToString() == "-1" || gridView1.GetRowCellValue(i, colStatus).ToString() == "3")
                            {
                                notbusyflag = false;
                            }
                            else if (gridView1.GetRowCellValue(i, colStatus).ToString() != "2")
                            {
                                notbusyflag = true;
                                break;
                            }
