        ...
        private void txtReturnPermission_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    {
                        query = "select customer_return_permission_details.Data_ID, customer_return_permission_details.Data_ID  as'الكود',Offer_Name  as 'البند',customer_return_permission_details.TotalQuantity as 'الكمية',sellprice.Last_Price as 'السعر',sellprice.Sell_Discount as 'نسبة الخصم',sellprice.Sell_Price as 'السعر بعد الخصم',(customer_return_permission_details.TotalQuantity*sellprice.Sell_Price) as 'الاجمالي','','' ,'" + "عرض" + "',customer_return_permission_details.Store_ID from customer_return_permission  inner join customer_return_permission_details on customer_return_permission.CustomerReturnPermission_ID=customer_return_permission_details.CustomerReturnPermission_ID inner join sellprice on sellprice.Data_ID=customer_return_permission_details.Data_ID inner join offer on offer.Offer_ID=customer_return_permission_details.Data_ID where customer_return_permission_details.CustomerReturnPermission_ID=" + txtReturnPermission.Text;
                    }
