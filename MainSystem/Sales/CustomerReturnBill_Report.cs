    ...
    public partial class CustomerReturnBill_Report : Form
    {
        ...
        public void DisplayBills()
        {
            try
            {
                ...
                string query = "SELECT customer_return_bill.CustomerReturnBill_ID AS 'التسلسل',branch.Branch_Name AS 'الفرع',customer_return_bill.Branch_BillNumber AS 'فاتورة رقم',customer_return_bill.Date AS 'التاريخ',customer_return_bill.TotalCostAD AS 'الاجمالى',customer_return_bill.ReturnInfo AS 'البيان',concat(customer_bill.Branch_BillNumber,' ',customer_bill.Branch_Name) as 'تابعة لفاتورة' FROM customer_return_bill INNER JOIN branch ON branch.Branch_ID = customer_return_bill.Branch_ID INNER JOIN customer_bill ON customer_return_bill.CustomerBill_ID = customer_bill.CustomerBill_ID where customer_return_bill.CustomerReturnBill_ID IN(" + q1 + ") order by customer_return_bill.CustomerReturnBill_ID";

                MySqlDataAdapter adapterSets = new MySqlDataAdapter(query, dbconnection);
                
                query = "SELECT customer_return_bill_details.CustomerReturnBill_ID as 'التسلسل',data.Code as 'الكود',customer_return_bill_details.TotalMeter as 'الكمية',customer_return_bill_details.PriceAD as 'السعر',customer_return_bill_details.SellDiscount as 'نسبة الخصم',customer_return_bill_details.TotalAD as 'الاجمالى',product.Product_Name as 'الصنف',color.Color_Name as 'اللون',size.Size_Value as 'المقاس',sort.Sort_Value as 'الفرز',data.Description as 'الوصف',data.Carton as 'الكرتنة',data_photo.Photo as 'صورة' FROM customer_return_bill_details INNER JOIN customer_return_bill ON customer_return_bill.CustomerReturnBill_ID = customer_return_bill_details.CustomerReturnBill_ID INNER JOIN data on data.Data_ID=customer_return_bill_details.Data_ID INNER JOIN type ON type.Type_ID = data.Type_ID INNER JOIN product ON product.Product_ID = data.Product_ID INNER JOIN factory ON data.Factory_ID = factory.Factory_ID INNER JOIN groupo ON data.Group_ID = groupo.Group_ID LEFT outer JOIN color ON data.Color_ID = color.Color_ID LEFT outer  JOIN size ON data.Size_ID = size.Size_ID LEFT outer  JOIN sort ON data.Sort_ID = sort.Sort_ID left join data_photo on data_photo.Data_ID=data.Data_ID where customer_return_bill_details.CustomerReturnBill_ID IN(" + q1 + ") order by data.Data_ID";
                ...
                adapterSets.Fill(dataSet11, "customer_return_bill");
