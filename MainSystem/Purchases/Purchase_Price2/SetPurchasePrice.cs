...
{
    public partial class SetPurchasePrice : Form
    {
        ...
        public SetPurchasePrice(ProductsPurchasePriceForm productsPurchasePriceForm, XtraTabControl xtraTabControlSalesContent)
        {
            try
            {
                InitializeComponent();
        ...
        //main events
        private void SetPurchasePrice_Load(object sender, EventArgs e)
        {
            try
            {
                dbconnection.Open();
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                if (gridView1.SelectedRowsCount>0)
                {
                        ...
                        if (gridView1.SelectedRowsCount > 1)
                        {
                            ...
                            for (int i = 0; i < dataTable.Rows.Count; i++)
                            {
                                if (gridView1.IsRowSelected(i))
                                {
                                    ...
                                    command.ExecuteNonQuery();
                                    //insert into additional_increas_PurchasePrice
                                    insertIntoAdditionalIncrease(ref PurchasePrice_ID, ref oldPurchasePrice_ID);
                    ...
                    UserControl.ItemRecord("purchasing_price", "اضافة", PurchasePrice_ID, DateTime.Now, "", dbconnection);
                    UserControl.ItemRecord("oldpurchasing_price", "اضافة", oldPurchasePrice_ID, DateTime.Now, "", dbconnection);
        ...
        public void displayProducts()
        {
            ...
            string query = "SELECT data.Data_ID,data.Code as 'الكود',product.Product_Name as 'الصنف',type.Type_Name as 'النوع',factory.Factory_Name as 'المصنع',groupo.Group_Name as 'المجموعة',color.Color_Name as 'اللون',size.Size_Value as 'المقاس',sort.Sort_Value as 'الفرز',data.Classification as 'التصنيف',data.Description as 'الوصف',data.Carton as 'الكرتنة' from data INNER JOIN type ON type.Type_ID = data.Type_ID INNER JOIN product ON product.Product_ID = data.Product_ID INNER JOIN factory ON data.Factory_ID = factory.Factory_ID INNER JOIN groupo ON data.Group_ID = groupo.Group_ID LEFT outer JOIN color ON data.Color_ID = color.Color_ID LEFT outer  JOIN size ON data.Size_ID = size.Size_ID LEFT outer  JOIN sort ON data.Sort_ID = sort.Sort_ID where  data.Type_ID IN(" + q1 + ") and  data.Factory_ID  IN(" + q2 + ") and  data.Product_ID  IN(" + q3 + ") and data.Group_ID IN (" + q4 + ") and data.Data_ID not in (" + getDataIDsWhichHavePurchasePrice() + ") " + fQuery + " order by SUBSTR(data.Code,1,16),color.Color_Name ,data.Sort_ID ";
            MySqlDataAdapter da = new MySqlDataAdapter(query, dbconnection);
            DataTable dt = new DataTable();
            da.Fill(dt);
        ...
        public void insertIntoAdditionalIncrease(ref int PurchasePrice_ID, ref  int oldPurchasePrice_ID)
        {
        ...
        public string getDataIDsWhichHavePurchasePrice()
        {
            dbconnection.Open();
            string query = "select Data_ID from purchasing_price";
            MySqlCommand com = new MySqlCommand(query, dbconnection);
