...
namespace MainSystem
{
    public partial class Order_Report : Form
    {
        ...
        public void search()
        {
            ...
            MySqlDataAdapter adapterOrder = new MySqlDataAdapter("select orders.Order_ID as 'التسلسل',factory.Factory_Name as 'الشركة',orders.Order_Number as 'رقم الفاتورة',orders.Employee_Name as 'الموظف المسئول',store.Store_Name as 'المخزن',supplier.Supplier_Name as 'المورد',orders.Request_Date as 'تاريخ الطلب',orders.Receive_Date as'تاريخ الاستلام' from orders inner join factory on factory.Factory_ID=orders.Factory_ID left join supplier on supplier.Supplier_ID=orders.Supplier_ID inner join store on store.Store_ID=orders.Store_ID where orders.Confirmed=1 and orders.Received=0 and orders.Canceled=0", conn);
