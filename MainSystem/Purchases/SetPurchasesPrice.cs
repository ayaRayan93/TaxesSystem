                ...
                if (txtCode.Text != "" || chBoxSelectAll.Checked)
                {
                    ...
                    if (radioQata3y.Checked == true)
                    {
                        ...
                        if (txtCode.Text == "" && chBoxSelectAll.Checked)
                        {
                            ...
                            {
                                string query = "INSERT INTO purchasing_price (Purchasing_Discount,Price_Type, Purchasing_Price, ProfitRatio, Data_ID, Price,Last_Price, PercentageDelegate,Date) VALUES(?Purchasing_Discount,?Price_Type,?Purchasing_Price,?ProfitRatio,?Data_ID,?Price,?Last_Price,?PercentageDelegate,?Date)";
                        ...
                        else
                        {
                            if (id != 0)
                            {
                                string query = "INSERT INTO purchasing_price (Purchasing_Discount,Price_Type,Last_Price, Purchasing_Price, ProfitRatio, Data_ID, Price, PercentageDelegate,Date) VALUES(?Purchasing_Discount,?Price_Type,?Last_Price,?Purchasing_Price,?ProfitRatio,?Data_ID,?Price,?PercentageDelegate,?Date)";
                                MySqlCommand command = new MySqlCommand(query, dbconnection);
                                command.Parameters.AddWithValue("?Price_Type", "قطعى");
                                command.Parameters.AddWithValue("?Purchasing_Price", calPurchasesPrice());
                    ...
                    else
                    {
                        ...
                        if (txtCode.Text == "" && chBoxSelectAll.Checked)
                        {
                            ...
                            for (int i = 0; i < dataTable.Rows.Count; i++)
                            {
                                ...
                                string query = "INSERT INTO purchasing_price (Last_Price,Price_Type,Purchasing_Price,Data_ID,Purchasing_Discount,Price,Normal_Increase,Categorical_Increase,PercentageDelegate,Date) VALUES (?Last_Price,?Price_Type,?Purchasing_Price,?Data_ID,?Purchasing_Discount,?Price,?Normal_Increase,?Categorical_Increase,?PercentageDelegate,?Date)";
                                MySqlCommand command = new MySqlCommand(query, dbconnection);
