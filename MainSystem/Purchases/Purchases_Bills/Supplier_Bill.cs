                ...
                if (double.TryParse(txtTotalMeter.Text, out quantity))
                {
                    ...
                    if (!IsAdded())
                    {
                        ...
                        if (gridView2.IsNewItemRow(rowHandle))
                        {
                            ...
                            gridView2.SetRowCellValue(rowHandle, gridView2.Columns["السعر"], txtPrice.Text);
                            if (row1["نوع السعر"].ToString() == "لستة")
                            {
                                gridView2.SetRowCellValue(rowHandle, gridView2.Columns["خصم الشراء"], txtDiscount.Text);
                            }
                            else if (row1["نوع السعر"].ToString() == "قطعى")
                            {
                                gridView2.SetRowCellValue(rowHandle, gridView2.Columns["نسبة الشراء"], txtDiscount.Text);
                            }
                            gridView2.SetRowCellValue(rowHandle, gridView2.Columns["الزيادة العادية"], txtNormalIncrease.Text);
                            gridView2.SetRowCellValue(rowHandle, gridView2.Columns["الزيادة القطعية"], txtCategoricalIncrease.Text);
                            gridView2.SetRowCellValue(rowHandle, gridView2.Columns["ضريبة القيمة المضافة"], txtTax.Text);
                            gridView2.SetRowCellValue(rowHandle, gridView2.Columns["سعر الشراء"], txtPurchasePrice.Text);
                            gridView2.SetRowCellValue(rowHandle, gridView2.Columns["اجمالى عدد الامتار"],txtTotalMeter.Text);
                            gridView2.SetRowCellValue(rowHandle, gridView2.Columns["PurchasingPrice_ID"], row1["PurchasingPrice_ID"].ToString());
                            gridView2.SetRowCellValue(rowHandle, gridView2.Columns["نوع السعر"], row1["نوع السعر"].ToString());
                            gridView2.SetRowCellValue(rowHandle, gridView2.Columns["Supplier_Permission_Details_ID"], row1["Supplier_Permission_Details_ID"].ToString());
                        }
                    }
                    else
                    {
        ...
        private void comPermessionNum_SelectedValueChanged(object sender, EventArgs e)
        {
            ...
            if (flag)
            {
                try
                {
                    ...
                    q = "SELECT DISTINCT data.Data_ID,data.Code AS 'الكود',concat(product.Product_Name,' - ',type.Type_Name,' - ',factory.Factory_Name,' - ',groupo.Group_Name,' ',COALESCE(color.Color_Name,''),' ',COALESCE(size.Size_Value,''),' ',COALESCE(sort.Sort_Value,'')) as 'الاسم',purchasing_price.Price AS 'السعر',purchasing_price.Purchasing_Discount AS 'خصم الشراء',purchasing_price.Normal_Increase AS 'الزيادة العادية',purchasing_price.Categorical_Increase AS 'الزيادة القطعية',purchasing_price.ProfitRatio as 'نسبة الشراء',purchasing_price.Purchasing_Price AS 'سعر الشراء',supplier_permission_details.Total_Meters as 'اجمالى عدد الامتار',purchasing_price.PurchasingPrice_ID,purchasing_price.Price_Type as 'نوع السعر',supplier_permission_details.Supplier_Permission_Details_ID,'ضريبة القيمة المضافة' FROM storage_import_permission INNER JOIN import_supplier_permission ON import_supplier_permission.StorageImportPermission_ID = storage_import_permission.StorageImportPermission_ID inner join supplier_permission_details on supplier_permission_details.ImportSupplierPermission_ID=import_supplier_permission.ImportSupplierPermission_ID INNER JOIN data ON supplier_permission_details.Data_ID = data.Data_ID INNER JOIN type ON type.Type_ID = data.Type_ID INNER JOIN product ON product.Product_ID = data.Product_ID INNER JOIN factory ON data.Factory_ID = factory.Factory_ID INNER JOIN groupo ON data.Group_ID = groupo.Group_ID LEFT JOIN color ON color.Color_ID = data.Color_ID LEFT JOIN size ON size.Size_ID = data.Size_ID LEFT JOIN sort ON sort.Sort_ID = data.Sort_ID left JOIN purchasing_price ON data.Data_ID = purchasing_price.Data_ID where storage_import_permission.StorageImportPermission_ID=0";
