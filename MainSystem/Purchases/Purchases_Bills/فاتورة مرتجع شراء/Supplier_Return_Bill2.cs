    ...
    public partial class Supplier_Return_Bill2 : Form
    {
        ...
        private void comSupplier_SelectedValueChanged(object sender, EventArgs e)
        {
            if (loadSup)
            {
                try
                {
                    string query = "SELECT import_storage_return_supplier.Supplier_Permission_Number,import_storage_return_supplier.ImportStorageReturnSupplier_ID FROM import_storage_return_supplier where import_storage_return_supplier.Supplier_ID=" + comSupplier.SelectedValue.ToString() + " and import_storage_return_supplier.ImportStorageReturn_ID=" + ImportStorageReturnId + " and import_storage_return_supplier.ReturnedPurchaseBill=0";
                    MySqlDataAdapter da = new MySqlDataAdapter(query, dbconnection);
                    DataTable dt = new DataTable();
                    da.Fill(dt);
            ...
            if (storageImportPermissionId > 0)
            {
                ...
                while (dr.Read())
                {
                    ...
                    if (gridView1.IsNewItemRow(rowHandl))
                    {
                        ...
                        gridView1.SetRowCellValue(rowHandl, gridView1.Columns["التاريخ"], dr["التاريخ"].ToString());
                        ...
                        //q = "SELECT    FROM supplier_bill INNER JOIN supplier_bill_details ON supplier_bill_details.Bill_ID = supplier_bill.Bill_ID INNER JOIN storage_import_permission ON supplier_bill.StorageImportPermission_ID = storage_import_permission.StorageImportPermission_ID INNER join import_supplier_permission on import_supplier_permission.StorageImportPermission_ID=storage_import_permission.StorageImportPermission_ID INNER join supplier_permission_details on supplier_permission_details.ImportSupplierPermission_ID=import_supplier_permission.ImportSupplierPermission_ID INNER JOIN import_storage_return ON import_storage_return.StorageImportPermission_ID = storage_import_permission.StorageImportPermission_ID INNER JOIN import_storage_return_supplier ON import_storage_return_supplier.ImportStorageReturn_ID = import_storage_return.ImportStorageReturn_ID INNER JOIN import_storage_return_details ON import_storage_return_details.ImportStorageReturnSupplier_ID = import_storage_return_supplier.ImportStorageReturnSupplier_ID and import_storage_return_details.Supplier_Permission_Details_ID=supplier_permission_details.Supplier_Permission_Details_ID INNER JOIN data ON data.Data_ID = import_storage_return_details.Data_ID and supplier_permission_details.Data_ID=supplier_bill_details.Data_ID and supplier_bill_details.Supplier_Permission_Details_ID=supplier_permission_details.Supplier_Permission_Details_ID INNER JOIN type ON type.Type_ID = data.Type_ID INNER JOIN product ON product.Product_ID = data.Product_ID INNER JOIN factory ON data.Factory_ID = factory.Factory_ID INNER JOIN groupo ON data.Group_ID = groupo.Group_ID LEFT JOIN color ON color.Color_ID = data.Color_ID LEFT JOIN size ON size.Size_ID = data.Size_ID LEFT JOIN sort ON sort.Sort_ID = data.Sort_ID where supplier_bill.StorageImportPermission_ID=" + storageImportPermissionId + " and import_storage_return_supplier.ImportStorageReturnSupplier_ID=" + comSupPerm.SelectedValue.ToString() + " and supplier_bill.Supplier_Permission_Number=" + comSupPerm.Text + " and supplier_bill_details.Data_ID=" + dr["Data_ID"].ToString();
        ...
        public void displayPermissionSupplier()
        {
            ...
            if (storageImportPermissionId > 0)
            {
                ...
                //    comSupplier.DisplayMember = dt.Columns["Supplier_Name"].ToString();
                //    comSupplier.ValueMember = dt.Columns["Supplier_ID"].ToString();
                string query = "select DISTINCT supplier.Supplier_ID,supplier.Supplier_Name from import_storage_return INNER JOIN import_storage_return_supplier ON import_storage_return_supplier.ImportStorageReturn_ID = import_storage_return.ImportStorageReturn_ID INNER JOIN supplier ON import_storage_return_supplier.Supplier_ID = supplier.Supplier_ID where import_storage_return_supplier.ReturnedPurchaseBill=0 and import_storage_return.ImportStorageReturn_ID=" + ImportStorageReturnId;
                MySqlDataAdapter da = new MySqlDataAdapter(query, dbconnection);
                DataTable dt = new DataTable();
                da.Fill(dt);
                comSupplier.DataSource = dt;
