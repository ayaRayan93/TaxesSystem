            ...
            try
            {
                if (txtExpenseNum.Text != "" && comBank.Text != "" && txtPullMoney.Text != "" && txtDescrip.Text != "" && txtClient.Text != "")
                {
                    ...
                    if (double.TryParse(txtPullMoney.Text, out outParse) && int.TryParse(txtExpenseNum.Text, out ExpenseNum))
                    {
                        ...
                        MySqlCommand com5 = new MySqlCommand("select Amount from expense_transition where ExpenseTransition_ID=" + txtExpenseNum.Text, dbconnection);
                        if (com5.ExecuteScalar() != null)
                        {
                        ...
                        {
                            MessageBox.Show("رقم المصروف غير موجود من قبل");
                        }
                    }
                    else
                    {
