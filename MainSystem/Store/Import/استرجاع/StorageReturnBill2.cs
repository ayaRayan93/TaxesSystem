                    ...
                    if (row1 != null /*&& comSupplier.SelectedValue != null*/ && txtPermissionNum.Text != "" && txtCarton.Text != "" && txtBalat.Text != "" && txtCode.Text != "" && txtTotalMeter.Text != "" && txtReason.Text != "")
                    {
                        ...
                        if (int.TryParse(txtBalat.Text, out NoBalatat) && int.TryParse(txtCarton.Text, out NoCartons) && int.TryParse(txtPermissionNum.Text, out permNum) && double.TryParse(txtTotalMeter.Text, out total))
                        {
                            ...
                            if (radioButtonReturnPermission.Checked)
                            {
                                ...
                                string query = "select ImportStorageReturn_ID from import_storage_return where Import_Permission_Number=" + permNum + " and Store_ID=" + comStore.SelectedValue.ToString();
                                ...
                                if (com.ExecuteScalar() == null)
                                {
                                    ...
                                    query = "SELECT gate.Car_ID,gate.Car_Number,gate.Driver_ID,gate.Driver_Name FROM gate INNER JOIN gate_permission ON gate_permission.Permission_Number = gate.Permission_Number where gate.Store_ID=" + comStore.SelectedValue.ToString() + " and gate.Supplier_ID=" + comSupplier.SelectedValue.ToString() + " and gate_permission.Supplier_PermissionNumber=" + row1["اذن استلام"] + " and gate_permission.Type='دخول'";
                                ...
                                query = "select Total_Meters from storage where Data_ID=" + row1["Data_ID"].ToString() + " and Store_ID=" + comStore.SelectedValue.ToString() + " and Store_Place_ID=" + row1["Store_Place_ID"].ToString();
                                com = new MySqlCommand(query, dbconnection);
                                if (com.ExecuteScalar() != null)
                                {
                                    double totalMeter = Convert.ToDouble(com.ExecuteScalar().ToString());

                                    query = "update storage set Total_Meters=" + (totalMeter - total) + " where Data_ID=" + row1["Data_ID"].ToString() + " and Store_ID=" + comStore.SelectedValue.ToString() + " and Store_Place_ID=" + row1["Store_Place_ID"].ToString();
                                    com = new MySqlCommand(query, dbconnection);
                                    com.ExecuteNonQuery();
                                }
                ...
                if (flagConfirm == 2)
                {
                    ...
                    if (row2 != null)
                    {
                        if (MessageBox.Show("هل انت متاكد انك تريد الحذف؟", "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                        {
                            ...
                            string query = "select Total_Meters from storage where Store_ID=" + comStore.SelectedValue.ToString() + " and Store_Place_ID=" + row2["Store_Place_ID"].ToString() + " and Data_ID=" + row2["Data_ID"].ToString();
                            MySqlCommand com = new MySqlCommand(query, dbconnection);
                            if (com.ExecuteScalar() != null)
                            {
                                ...
                                query = "update storage set Total_Meters=" + (totalf + Convert.ToDouble(row2["متر/قطعة"].ToString())) + " where Store_ID=" + comStore.SelectedValue.ToString() + " and Store_Place_ID=" + row2["Store_Place_ID"].ToString() + " and Data_ID=" + row2["Data_ID"].ToString();
                                com = new MySqlCommand(query, dbconnection);
                                com.ExecuteNonQuery();
                                dbconnection.Close();
        ...
        private void txtPermissionNum_TextChanged(object sender, EventArgs e)
        {
            try
            {
                if (txtPermissionNum.Text != "")
                {
                    ...
                    q = "select storage_import_permission.Confirmed from storage_import_permission where storage_import_permission.Import_Permission_Number=" + txtPermissionNum.Text + " and storage_import_permission.Store_ID=" + comStore.SelectedValue.ToString();
