...
{
    public partial class PermissionsDeliveryBills : Form
    {
        private MySqlConnection dbconnection, dbconnectionr;
        bool loaded = false;
        MainForm MainForm;

        public PermissionsDeliveryBills(MainForm MainForm)
        {
            try
            {
                InitializeComponent();
        ...
        private void btnReport_Click(object sender, EventArgs e)
        {
            try
            {
                MainForm.bindReportPermissionForm(gridControl2, "تقرير أذونات لم يتم تسليمها");
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
