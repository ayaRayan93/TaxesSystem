    ...
    public partial class CustomerDeliveryReport : Form
    {
        ...
        public void displayProducts()
        {
            try
            {
                ...
                string query = "SELECT Customer_Permissin_ID, customer_permissions.Customer_Permissin_ID as 'رقم الأذن',branch.Branch_Name as 'الفرع',customer_permissions.BranchBillNumber as 'رقم الفاتورة',customer_permissions.Customer_Name as 'مهندس/مقاول/تاجر',GROUP_CONCAT(c1.Phone)as 'تلفون م/م/ت',customer_permissions.Client_Name as 'العميل',GROUP_CONCAT(c2.Phone)as 'تلفون العميل', store.Store_Name as 'المخزن',StoreKeeper as 'أمين المخزن',DeliveredPerson as 'المستلم',Note as 'الملاحظة',customer_permissions.Date as 'التاريخ' FROM customer_permissions left join customer_phone as c1 on customer_permissions.Client_ID=c1.Customer_ID left join customer_phone as c2 on customer_permissions.Customer_ID=c2.Customer_ID INNER JOIN store ON store.Store_ID = customer_permissions.Store_ID inner join branch on branch.Branch_ID=customer_permissions.Branch_ID   WHERE " + subQuery+ " group by customer_permissions.Customer_Permissin_ID";
