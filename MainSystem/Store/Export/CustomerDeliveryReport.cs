    ...
    public partial class CustomerDeliveryReport : Form
    {
        ...
        public void displayProducts()
        {
            try
            {
                ...
                string query = "SELECT Customer_Permissin_ID, customer_permissions.Customer_Permissin_ID as 'رقم الأذن',branch.Branch_Name as 'الفرع',customer_permissions.BranchBillNumber as 'رقم الفاتورة', store.Store_Name as 'المخزن',StoreKeeper as 'أمين المخزن',DeliveredPerson as 'المستلم',Note as 'الملاحظة',customer_permissions.Date as 'التاريخ' FROM customer_permissions INNER JOIN store ON store.Store_ID = customer_permissions.Store_ID inner join branch on branch.Branch_ID=customer_permissions.Branch_ID   WHERE " + subQuery;
