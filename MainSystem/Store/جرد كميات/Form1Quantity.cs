        ...
        private void txtBillNumber_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                ...
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    {
                        string query = "select * from customer_bill  where customer_bill.Branch_BillNumber=" + BranchBillNum + " and Branch_ID="+ branchID;
                        MySqlCommand com = new MySqlCommand(query, dbconnection);
                        MySqlDataReader dr = com.ExecuteReader();
                        ...
                        {
                            flag2 = true;
                            billNum= Convert.ToInt16(dr["Dash_Bill_ID"]);
                            ...
                            dateTimePicker2.Value = Convert.ToDateTime(dr["Bill_Date"]);
                        }
                        dr.Close();
                        if (flag2 == true)
                        {
                            ...
                            //  query = "select distinct product_bill.Code as 'كود',product_bill.Quantity as ' الكمية',storage.Store_Place as 'مكان التخزين', type.Type_Name as 'النوع', factory.Factory_Name as 'المصنع' ,groupo.Group_Name as 'المجموعة', product.Product_Name as 'المنتج' ,data.Colour as 'اللون', data.Size as 'المقاس', data.Sort as 'الفرز',data.Classification as 'التصنيف', data.Description as 'الوصف'  from product_bill inner join data on data.Code=product_bill.Code INNER JOIN type ON type.Type_ID = data.Type_ID INNER JOIN product ON product.Product_ID = data.Product_ID INNER JOIN factory ON data.Factory_ID = factory.Factory_ID INNER JOIN groupo ON data.Group_ID = groupo.Group_ID left join storage on storage.Code=product_bill.Code  where product_bill.Dash_Bill_ID=" + billNum+ " and storage.Store_ID="+storeId+ " and storage.Storage_Date=(select Storage_Date from storage inner join product_bill on storage.Code=product_bill.Code where product_bill.Dash_Bill_ID=" + billNum + " and storage.Store_ID=" + storeId + " order by Storage_Date limit 1)";
                            query = "select distinct product_bill.Code as 'كود',product_bill.Quantity as ' الكمية',storage.Store_Place as 'مكان التخزين', type.Type_Name as 'النوع', factory.Factory_Name as 'المصنع' ,groupo.Group_Name as 'المجموعة', product.Product_Name as 'المنتج' ,data.Colour as 'اللون', data.Size as 'المقاس', data.Sort as 'الفرز',data.Classification as 'التصنيف', data.Description as 'الوصف'  from product_bill inner join data on data.Code=product_bill.Code INNER JOIN type ON type.Type_ID = data.Type_ID INNER JOIN product ON product.Product_ID = data.Product_ID INNER JOIN factory ON data.Factory_ID = factory.Factory_ID INNER JOIN groupo ON data.Group_ID = groupo.Group_ID left join storage on storage.Code=product_bill.Code  where product_bill.Dash_Bill_ID=" + billNum+ "  and storage.Storage_Date=(select Storage_Date from storage inner join product_bill on storage.Code=product_bill.Code where product_bill.Dash_Bill_ID=" + billNum + "  order by Storage_Date limit 1)";
                ...
                if (!finish)
                {
                    if (!exist)
                    {
                        if (row1 != null && txtRecivedQuantity.Text != "" && txtCode.Text != "")
                        {
                            ...
                            string query = "select sum(Received_Quantity) from received_bill_store where Branch_ID=" + branchID + " and Branch_BillNumber=" + BranchBillNum + " and Code=" + txtCode.Text + " and Status=0";
                            ...
                            if (double.TryParse(txtRecivedQuantity.Text, out recivedQuantity) && double.TryParse(row1.Cells[1].Value.ToString(), out totalQuantity))
                            {
                                ...
                                else if (totalQuantity > totalrecivedQuantity)
                                {
                                    ...
                                    dataGridView1.Rows[n].DefaultCellStyle.BackColor = Color.Silver;
                                    dataGridView2.Rows[n].Cells[1].Value = txtRecivedQuantity.Text;
