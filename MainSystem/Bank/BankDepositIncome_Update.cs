...
{
    public partial class BankDepositIncome_Update : Form
    {
        ...
        int[] arrPaidMoney;
        bool loaded = false;
        public static bool updateBankDepositIncomeTextChangedFlag = false;
        DataRowView selRow;
        XtraTabPage xtraTabPage;

        public BankDepositIncome_Update(DataRowView SelRow)
        {
            InitializeComponent();
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    check = (cmbIncomeType.Text != "" && txtClient.Text != "" && cmbBank.Text != "" && txtPaidMoney.Text != "" && txtCheckNumber.Text != "" && txtVisaType.Text != "" && txtOperationNumber.Text != "");
                }
                    ...
                    if (double.TryParse(txtPaidMoney.Text, out outParse))
                    {
                        ...
                        if (prompt.ShowDialog() == DialogResult.OK)
                        {
                            if (textBox.Text != "")
                            {
                                ...
                                updateBankDepositIncomeTextChangedFlag = false;
                                xtraTabPage.ImageOptions.Image = null;
                                //Main.DepositIncomeShow.search();
                                MainForm.tabControlBank.TabPages.Remove(BankDepositIncome_Report.MainTabPageUpdateDepositIncome);
                            }
                            else
                            {
        ...
        private void txtBox_TextChanged(object sender, EventArgs e)
        {
            try
            {
                if (loaded)
                {
                    xtraTabPage = getTabPage("tabPageUpdateDepositIncome");
                    if (!IsClear())
                    {
                        xtraTabPage.ImageOptions.Image = Properties.Resources.unsave;
                        updateBankDepositIncomeTextChangedFlag = true;
                    }
                    else
                    {
                        xtraTabPage.ImageOptions.Image = null;
        ...
        private void loadBranch()
        {
            ...
            cmbIncomeType.Text = selRow[14].ToString();
            dbconnection.Close();

            txtClient.Text = selRow[8].ToString();
        ...
        {
            dbconnection.Open();
            string query = "update income set IncomeType_ID,IncomeType where Income_ID=" + selRow[1].ToString();
            MySqlCommand com = new MySqlCommand(query, dbconnection);
