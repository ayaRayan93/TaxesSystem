...
{
    public partial class BankPullAgl_Update : Form
    {
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    check = (cmbBank.Text != "" && txtPullMoney.Text != "");
                }
                    ...
                    if (double.TryParse(txtPullMoney.Text, out outParse))
                    {
                        ...
                        if (prompt.ShowDialog() == DialogResult.OK)
                        {
                            if (textBox.Text != "")
                            {
                                ...
                                string query = "update Transitions set Amount=@Amount,Date=@Date,Data=@Data,PayDay=@PayDay,Check_Number=@Check_Number,Visa_Type=@Visa_Type,Operation_Number=@Operation_Number,Delegate_ID=@Delegate_ID,Delegate_Name=@Delegate_Name where Transition_ID=" + selRow[0].ToString();
        ...
        private void loadBranch()
        {
            ...
            if (CustomerType == "عميل")
            {
                radClient.Checked = true;
            }
            else if (CustomerType == "مهندس")
            {
                radEng.Checked = true;
            }
        ...
        {
            dbconnection.Open();
            string query = "select Money from customer_accounts where Client_ID=" + selRow["Client_ID"].ToString();
            MySqlCommand com = new MySqlCommand(query, dbconnection);
            if (com.ExecuteScalar() != null)
            {
                ...
                query = "update customer_accounts set Money=" + (restMoney + paidMoney) + " where Client_ID=" + selRow["Client_ID"].ToString();
            }
            else
            {
        ...
        void printCategoriesBill()
        {
            ...
            if (comClient.Text != "")
            {
                f.PrintInvoice(Convert.ToDateTime(dateTimePicker1.Value.ToString("yyyy-MM-dd HH:mm:ss")), selRow[0].ToString(), UserControl.EmpBranchName, comClient.Text + " " + comClient.SelectedValue.ToString(), Convert.ToDouble(txtPullMoney.Text), PaymentMethod, cmbBank.Text, txtCheckNumber.Text, dateEdit1.Text, txtVisaType.Text, txtOperationNumber.Text, txtDescrip.Text, selRow["الموظف"].ToString(), comDelegate.Text, arrPaidMoney[0], arrPaidMoney[1], arrPaidMoney[2], arrPaidMoney[3], arrPaidMoney[4], arrPaidMoney[5], arrPaidMoney[6], arrPaidMoney[7], arrPaidMoney[8], arrRestMoney[0], arrRestMoney[1], arrRestMoney[2], arrRestMoney[3], arrRestMoney[4], arrRestMoney[5], arrRestMoney[6], arrRestMoney[7], arrRestMoney[8]);
            }
            else if (comEng.Text != "")
            {
                f.PrintInvoice(Convert.ToDateTime(dateTimePicker1.Value.ToString("yyyy-MM-dd HH:mm:ss")), selRow[0].ToString(), UserControl.EmpBranchName, comEng.Text + " " + comEng.SelectedValue.ToString(), Convert.ToDouble(txtPullMoney.Text), PaymentMethod, cmbBank.Text, txtCheckNumber.Text, dateEdit1.Text, txtVisaType.Text, txtOperationNumber.Text, txtDescrip.Text, selRow["الموظف"].ToString(), comDelegate.Text, arrPaidMoney[0], arrPaidMoney[1], arrPaidMoney[2], arrPaidMoney[3], arrPaidMoney[4], arrPaidMoney[5], arrPaidMoney[6], arrPaidMoney[7], arrPaidMoney[8], arrRestMoney[0], arrRestMoney[1], arrRestMoney[2], arrRestMoney[3], arrRestMoney[4], arrRestMoney[5], arrRestMoney[6], arrRestMoney[7], arrRestMoney[8]);
            }
            f.ShowDialog();
            for (int i = 0; i < arrPaidMoney.Length; i++)
                arrPaidMoney[i] = arrRestMoney[i] = 0;
